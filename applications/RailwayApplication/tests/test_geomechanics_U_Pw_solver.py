...
from KratosMultiphysics.RailwayApplication.geomechanics_U_Pw_solver import UPwUvecSolver
...
class KratosRailwayUPwSolverTests(KratosUnittest.TestCase):
    ...
    def test_ConstructSolver_invalid_time_stepping(self):
        ...
        with self.assertRaisesRegex(ValueError,
                                    "The time step is not a multiple of the total time. Please adjust the time step."):
    ...
    def test_ConstructSolver_valid_time_stepping(self):
        """
        ...
        with self.assertRaisesRegex(ValueError,
                                 "The selected strategy, newton_raphson_linear_elastic_with_uvec, "
                                 "is only available for the U-Pw solver, dynamic solution type and newmark scheme"):
            uvec_solver._create_solving_strategy(None,"newton_raphson_linear_elastic_with_uvec" )



    def test_KeepAdvancingSolutionLoop(self):
        """
        ...
        self.assertTrue(uvec_solver.KeepAdvancingSolutionLoop(0.3))
        ...
        self.assertFalse(uvec_solver.KeepAdvancingSolutionLoop(0.3))
        ...
        self.assertFalse(uvec_solver.KeepAdvancingSolutionLoop(0.1 + 0.2))

    def test_PrepareModelPart(self):
        """
        ...
        self.assertVectorAlmostEqual(calculated_velocities, [1.0,2.0,3.0])
        self.assertVectorAlmostEqual(calculated_accelerations, [4.0,5.0,6.0])
        self.assertVectorAlmostEqual(calculated_angular_velocities, [1.0,2.0,3.0])
        self.assertVectorAlmostEqual(calculated_angular_accelerations, [4.0,5.0,6.0])
        ...
        self.assertVectorAlmostEqual(calculated_velocities, [0.0,0.0,0.0])
        self.assertVectorAlmostEqual(calculated_accelerations, [0.0,0.0,0.0])
        self.assertVectorAlmostEqual(calculated_angular_velocities, [0.0,0.0,0.0])
        self.assertVectorAlmostEqual(calculated_angular_accelerations, [0.0,0.0,0.0])
